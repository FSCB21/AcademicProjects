...
function NuevoProducto() {
    ...
    document.getElementById('productoCarro').innerHTML += '<p>' + Articulo + ' Valor(Con Iva): ' + PrecioIva + ' //Cantidad: ' + Cantidad + '</p>';
}
...
function Facturar() {
    ...
    for (i = 0; i < ProductosRegistrados.length; i++) {
        ...
        ProductosRegistrados[i]["Subtotal"] = ProductosRegistrados[i]["TotalConIvaCanti"] / (1 + (ValorIva / 100));

        ProductosRegistrados[i]["ValorIva"] = ProductosRegistrados[i]["TotalConIvaCanti"] - ProductosRegistrados[i]["Subtotal"];
        ...
        TotalDelTotalConIva += parseFloat(ProductosRegistrados[i]["TotalConIvaCanti"]);
    }
